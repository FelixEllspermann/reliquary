...
| Extra Deck summon | `Extra Summon.dc.html` | 10.1 s | The vault opens and a Reliquary monster comes out |
...
# 6 · Extra Deck summon (10.1 s)

Reliquary monsters are not played from the hand — they come out of the vault. The summon **is**
the vault opening, which is why this animation is longer and louder than an ordinary activation.

## Scenes

| # | Scene | Dur | `align` | `open` | card | vault |
|---|---|---|---|---|---|---|
| 1 | **Call** | 1.7 s | 0 | 0 | — | forms, 0 → 1 |
| 2 | **Unlock** | 1.8 s | 0 → 1 | 0 | — | present |
| 3 | **Open** | 1.5 s | 1 | 0 → 1 | — | splits, 1 → 0 |
| 4 | **Emerge** | 1.9 s | 1 | 1 | rises and flips | gone |
| 5 | **Present** | 3.2 s | 1 | 1 | hero → field zone | gone |

Vault centre **(640, 402)**, ring radius **r = 213**. Extra deck slot at (906, 452); the summoned
monster settles into the zone at (568, 452).

**Geometry constraint that decides r:** an aligned ring is a 45°-rotated square of side `2r`,
so it needs `2·r·√2` px of vertical room — 602 px at r = 213. The usable band is y 100 (just
under the label) to y 704. At r = 250 the ring needs 707 px and its top vertex is cut off the
canvas — and because the rings are rotated *off* 45° until they align, the clipping appears
exactly at the climax. Derive r from the frame, not from how big the door should look.

## Call
The extra deck slot brightens and a 2 px light line runs from it to the vault centre
(`length × pull`), then the seal is born out of that line. Fourteen motes drift **inward** on
fixed angles. Label: `SUMMONING` / `Level 3 · two tributes paid` — the cost is already paid
before the animation starts.

## Unlock
Three diamond rings at scale 1.0 / 0.74 / 0.5, each starting at a rotation offset
(−46° / +38° / −62°) and easing to 0 — **arriving at alignment is the beat**. Four locks travel
from `r × 1.5` to `r × 0.98` and bite with a `pop` at `seg(p, 0.62, 0.94)`, with a small screen
shake on the last 22 %. Ring opacity rises 0.35 → 0.95 with alignment, so the door visibly
becomes solid as it locks.

## Open
The plate splits into **four quadrant clips of one diamond** — the same technique as the seal
shatter in §1 — flying along their diagonals to `r × 1.15` and rotating ±12°. The rings scale
outward (`1 + open × (0.6 + i × 0.3)`) and fade; that expansion past the frame is intentional.
Light floods from a radial core, a diamond shock ring sweeps to 1180 px, and the field un-dims
from 0.70 to 0.40.

## Emerge
```
rise = enter(seg(p, 0.08, 0.82))
y     = lerp(VAULT.y + 96, VAULT.y − 4, rise)
scale = lerp(0.40, 1, rise)
flip  = drift(seg(p, 0.30, 0.90)) × 180°      // back → front, mid-rise
glow  = lerp(0.90, 0.50, rise)
```
The **shaft of light** the card rides: 340 px tall, anchored at the vault and drawn upward,
`linear-gradient(180deg, transparent, #EFE7FA@30%)` so it is brightest **at the vault end**
(`0deg` puts the last stop at the top and inverts it), tapered with
`clip-path: polygon(40% 0, 60% 0, 100% 100%, 0 100%)` and softened at the sides with
`mask-image: linear-gradient(90deg, transparent, #000 34%, #000 66%, transparent)`. Without the
taper and mask it reads as a grey box floating above the card.

## Present
Banner `RELIQUARY SUMMON` / **Special Summon**, three stat chips, then the card travels to its
field zone and scales from hero (216 × 302) to field size (132 × 185) over
`enter(seg(p, 0.60, 0.94))`. The banner and chips fade as it settles — the hero framing has
done its job by then. The extra-deck caption fades to 0 during this scene; the chip row is
centred on the vault and reaches x 973, which would otherwise sit on it.

## The Reliquary card frame
A Reliquary monster is visually distinct from a normal monster, so a player can tell where it
came from at a glance:

| Normal monster | Reliquary monster |
|---|---|
| edge `#C8A45C` (or type colour) | edge **`#EFE7FA`**, 3 px |
| name plate `#42301C → #22150A` | `#3A2B4E → #1C1428`, violet rules |
| type band `MONSTER` in gold | **`RELIQUARY`** in `#EFE7FA → #5E4E8C` |
| level crest gold | crest `#EFE7FA → #5E4E8C`, violet numeral |
| card back gold weave | violet weave `#2A2148 → #0D0916` |

The artwork frame stays gold — that is the one piece of continuity with the rest of the set.
...
// Extra Deck summon
public IEnumerator PlayReliquarySummon(CardId card, int zone, int tributes);
public event Action OnVaultOpened;       // end of Open — the card may be instantiated
```
...
| `Extra Summon.dc.html` + `extra-summon.jsx` | Vault opening and the Reliquary summon |
