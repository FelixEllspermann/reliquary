...
- **Deck-Statistiken** (seit 2026-08-11): Der Server verbucht jedes Match als Aggregat je Deck-Zusammensetzung (SQLite-Tabelle `deck_stats`, Identität = SHA1 über Held + sortierte Kartenlisten; `db.recordDeckResult`/`db.topDecks`). PvP fließt autoritativ am `op:'end'` des DuelHost ein (Deck-Snapshots liegen ab `startServerDuel` am Duell), Solo/Turm/Draft meldet der Client über `solo_result` mit `deckName`/`deckHero`/`deckCards`/`deckExtra` (hinter dem Belohnungs-Cooldown, max. 1 Tracking/Minute). Abfrage: `stats_decks` → `NetMessage.decks` (`StatsDeck[]` mit `cards`/`extra` als `{n,c}`-Paaren). Anzeige: dritter Tab **STATISTICS** in DeckEditor/Shop → Szene `Statistics.unity` (`StatisticsController`).
