...
- **Archetypes** (seit 2026-08-12): `ArchetypeCatalog.Names` (Client) und `ARCHETYPES` (server.js) führen dieselbe kuratierte Liste der 29 Familien — Zugehörigkeit per Namens-Präfix (`StartsWith`; „Dragon Shrine" fängt keine „Dragon Claw"-Generics). Wer einen Archetype ergänzt, pflegt BEIDE Listen. Ein Deck „spielt" einen Archetype ab 4 Karten (`archetypesOf` in server.js) — Grundlage für `archetype_stats`/`archetype_pairs` (nur Online) und den `stats_archetypes`-Endpoint. Im Deck Builder filtert ein Archetype-Dropdown (zweite Toolbar-Zeile), die STATISTICS-Seite hat den CARDS|ARCHETYPES-Umschalter samt Kartenart-/Rarity-Chips.
- **Match-Statistiken** (seit 2026-08-11): Ein Eintrittspunkt `db.recordDeckResult` verbucht jedes Match dreifach — `deck_stats` (Aggregat je Deck-Zusammensetzung, SHA1 über Held + sortierte Karten), `card_stats` (je Karte EIN Zähler pro Match, Kopien egal) und `card_pairs` (alphabetisch normierte Paare für „often paired with"). Dazu `match_log` je Konto (`db.recordMatch`) für die Profil-Historie. PvP fließt autoritativ am `op:'end'` des DuelHost ein (Deck-Snapshots liegen ab `startServerDuel` am Duell), Solo/Turm/Draft meldet der Client über `solo_result` mit `deckName`/`deckHero`/`deckCards`/`deckExtra`/`opponent` (hinter dem Belohnungs-Cooldown). **Die Karten-Statistik wertet NUR Online-Matches**: `card_pairs` wird nur bei `pvp` beschrieben, `topCards` filtert `pvp_games > 0`, und die STATISTICS-Seite zeigt die PvP-Bilanz (Solo verzerrt gegen Bots; er zählt weiter in `deck_stats`, `card_stats.games` und der Profil-Historie). Abfragen: `stats_cards` (Top-Karten mit Bilanz), `stats_card_detail` (Partner einer Karte), `stats_decks` (Deck-Aggregate, aktuell ohne UI), `profile_stats` (Historie + Bilanzen + `liveGames`). Anzeige: Tab **STATISTICS** (Karten-Winrates + Pairings, `Statistics.unity`) und die Profil-Seite.
- **Profil-Seite** (`Profile.unity`, `ProfileController`): eigenständige Szene statt Overlay — Rang/Saison, Lifetime-Bilanz, Sammlungszähler, die letzten 20 Spiele und laufende Duelle mit WATCH. Der Miniprofil-Klick im Hauptmenü lädt die Szene; Kosmetik/Titel öffnet dort der CUSTOMIZE-Knopf als bisheriges `ProfilePanel`-Overlay.
- **Zuschauer-Modus**: Der DuelHost sendet zu jedem State/Event zusätzlich eine `to:"S"`-Fassung mit `viewer=null` (beide Hände verdeckt — kein Ghosting); `op:'poke'` erzwingt einen State für Neuankömmlinge. Node verteilt `to=="S"` und `to==null` an `duel.spectators` (`spectate`/`spectate_leave`, Austrag bei `end`). Client: `MatchContext.SpectateMode` — der Zuschauer lädt die Duel-Szene als reiner Spiegel (A unten), Requests erreichen ihn nie, `ServerDuelClient.OnDestroy` meldet ihn ab.
...
- **NEW-Karten** (seit 2026-08-12): `grantCard(acc, name, finish)` im Server ist der EINE Weg, Karten zu gewähren (Pack, Craft, Admin-Editor) — Erstbesitz landet in `acc.progress.newCards` und reist als `newCards` im Profil. Der Starter läuft bewusst an `grantCard` vorbei (selbst gewählt = nicht „neu"). Client: `PlayerProfile.NewCards`/`IsNew`/`MarkSeen`; der Deck Builder zeigt ein goldenes NEW-Plättchen auf der Pool-Kachel und hat NEW als vierten Besitz-Chip. Anklicken (Select) nimmt das Badge sofort, meldet `seen_card` und lässt die Kachel bis zum nächsten Rebuild stehen, damit unter dem NEW-Filter nichts unterm Zeiger wegspringt.
